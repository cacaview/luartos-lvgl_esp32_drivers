...
disp_backlight_h disp_backlight_new(const disp_backlight_config_t *config)
{
    ...
    if (config->pwm_control){
        ...
        // GPIO matrix configuration is handled automatically by ledc_channel_config
    }
    else
    {
        ...
        // Set initial output level based on invert setting
        ESP_ERROR_CHECK(gpio_set_level(config->gpio_num, config->output_invert ? 0 : 1));
    }
